...
// ESP32-C3 doesn't support ext1 wakeup, but ESP32-C6 does
...
void DeepSleepComponent::deep_sleep_() {
  // ESP32-C3 and ESP32-C6 use different APIs for GPIO wakeup
#if !defined(USE_ESP32_VARIANT_ESP32C3) && !defined(USE_ESP32_VARIANT_ESP32C6)
  // Original ESP32, S2, S3 variants
...
#if defined(USE_ESP32_VARIANT_ESP32C3) || defined(USE_ESP32_VARIANT_ESP32C6)
  // ESP32-C3 and ESP32-C6 variants
  if (this->sleep_duration_.has_value())
    esp_sleep_enable_timer_wakeup(*this->sleep_duration_);
  
  // Single pin wakeup using GPIO wakeup API
  if (this->wakeup_pin_ != nullptr) {
    bool level = !this->wakeup_pin_->is_inverted();
    if (this->wakeup_pin_mode_ == WAKEUP_PIN_MODE_INVERT_WAKEUP && this->wakeup_pin_->digital_read()) {
      level = !level;
  ...
  // ESP32-C6 supports ext1 wakeup (multiple pins)
