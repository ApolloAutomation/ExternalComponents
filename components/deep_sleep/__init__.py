...
def validate_config(config):
    # ESP32-C3 doesn't support touch wakeup
    if get_esp32_variant() == VARIANT_ESP32C3 and CONF_TOUCH_WAKEUP in config:
        raise cv.Invalid("ESP32-C3 does not support wakeup from touch.")
    # Note: ESP32-C6 DOES support ext1 wakeup, so we don't exclude it here
    return config
...
async def to_code(config):
    ...
    cg.add_build_flag("-DCUSTOM_DEEP_SLEEP_COMPONENT")
    import logging
    _LOGGER = logging.getLogger(__name__)
    _LOGGER.info("Custom deep_sleep component is being loaded")
